...
icurl -kG https://localhost/api/class/ctrlrInst.json -d 'rsp-subtree=full' -d 'rsp-prop-include=config-only' > /tmp/vetr-collector/ctrlrInst.json
icurl -kG https://localhost/api/class/fabricInst.json -d 'rsp-subtree=full' -d 'rsp-prop-include=config-only' > /tmp/vetr-collector/fabricInst.json
icurl -kG https://localhost/api/class/infraInfra.json -d 'rsp-subtree=full' -d 'rsp-prop-include=config-only' > /tmp/vetr-collector/infraInfra.json
icurl -kG https://localhost/api/class/fvTenant.json -d 'rsp-subtree=full' -d 'rsp-prop-include=config-only' > /tmp/vetr-collector/fvTenant.json
icurl -kG https://localhost/api/class/vmmDomP.json -d 'rsp-subtree=full' -d 'rsp-prop-include=config-only' > /tmp/vetr-collector/vmmDomP.json
...
echo Please provice aci-vetr-data.zip to Cisco Services for analysis.
